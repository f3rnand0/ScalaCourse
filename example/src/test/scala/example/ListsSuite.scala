...
class ListsSuite extends FunSuite {
  ...
  test("sum of numbers including negative and zero") {
    assert(sum(List(3, -7, 0)) === -4)
  }

  test("sum of numbers of empty List") {
    assert(sum(List()) === 0)
  }
  ...
  test("max of numbers including negative and zero") {
    assert(max(List(-3, 0, 1)) === 1)
  }

  test("max of numbers of empty List") {
